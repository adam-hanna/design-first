...
export const Validate = async <T>(type: any, payload: any, skipMissingProperties = false): Promise<void> => {
  ...
  if (errors && errors.length > 0) {
    throw errors.map((error: ValidationError) => Object.values(error.constraints)).join(', ');
  }
}
